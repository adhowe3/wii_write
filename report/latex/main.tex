...
The Wii remote is connected to the Raspberry Pi over Bluetooth through BlueZ, a native Linux Bluetooth stack. This connection manages the legacy Bluetooth signal from the Wii remote and opens a Logical Link Control and Adaptation Protocol (L2CAP) transport pipe. Our python script for reading the accelerometer data from the Wii remote could interface directly to this stream using a raw Bluetooth socket, but that would bog down the Raspberry Pi's CPU since it would be getting 100 accelerometer readings per second and we would have to manually decode the hexadecimal payloads from the Wii remote. To optimize the stream of data to allow enough resources for the machine learning model to run on the Raspberry Pi, we abstract the handling of the Bluetooth packets to the Linux kernel with the hid-wiimote kernel driver. This translates the Nintendo-specific hexadecimal payloads into standard Linux inputs. Once the payloads are in a format that Linux understands, the evdev (event device) subsystem filters the stream of Bluetooth data and only registers unique events that out python script uses to collect that data for processing. 
...
\begin{table*}[b] % The asterisk makes it span both columns. [t] puts it at the top of a page.
    \centering
    ...
    \begin{tabular}{cccc}
        ...
        \textbf{(Feet)} & \textbf{Packet Loss (out of 50)} & \textbf{Rate (Hz)} & \textbf{Accuracy (\%)} \\
        \midrule
        1  & 0  & $\sim$75 & 100 \\
        2  & 0  & $\sim$75 & 100 \\
        5  & 0  & $\sim$75 & 100 \\
...
Individually, the two models were each about 90\% accurate, but when combine they reached 93-97\% on the test data. The reason for the range of accuracy, is because each time the MLP model was trained, it gave a different accuracy. TODO: what model accuracy did we end up saving in the .pkl?
...
To evaluate the maximum effective range of the system, we tested packet reception and prediction accuracy at varying distances. First, we tested the reliability of a discrete button press signal using a custom script to count detected events. We pressed the 'A' button 50 times at distances ranging from 1 to 90 feet. The system recorded zero packet loss up to 80 feet; at 90 feet, 15 out of 50 button presses were lost. Next, we evaluated continuous accelerometer telemetry to compare the received packet rate against the Wii remote’s 100 Hz transmission rate. Because the evdev subsystem filters non-unique accelerometer data, the remote was continuously shaken to ensure a constant stream of unique spatial events. We recorded the number of packets received during the duration of an 'A' button hold. Due to the Time-Division Multiplexing (TDM) collision on the Raspberry Pi’s shared Wi-Fi and Bluetooth antenna, our maximum receivable baseline was $\sim$75 Hz. Testing at the same distance intervals, we maintained this $\sim$75 Hz baseline up to 70 feet. At 80 feet, throughput fell to $\sim$50 Hz, and degraded to as low as 5 Hz at 90 feet. Finally, we tested the maximum distance for accurate ML inference. Using the letter 'L' (which achieved 100\% accuracy in our 2-foot baseline tests), we performed 5 gesture writes at each distance. The model successfully maintained 100\% prediction accuracy up to 80 feet, before sharply declining to 20\% accuracy at 90 feet.  
